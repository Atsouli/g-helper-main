...
namespace GHelper.Input
{
    ...
    public class InputDispatcher
    {
        ...
        public void RegisterKeys()
        {
            ...
            if (AppConfig.IsAlly())
            {
                // Ally firmware emits F17 while the ROG key is held and F18 when it is released.
                hook.RegisterHotKey(ModifierKeys.None, Keys.F17);
                hook.RegisterHotKey(ModifierKeys.None, Keys.F18);
        ...
        public static int[] ParseHexValues(string input)
        {
            ...
            if (!Regex.IsMatch(input, $"^{pattern}((\\s+|\\s*\\+\\s*){pattern})*$")) return new int[0];
        ...
        internal static void SetBrightness(bool up, bool hotkey = false)
        {
            int brightness = -1;
        ...
        public static void KeyProcess(string name = "m3")
        {
            if (isPaddlePressed)
            {
                string secondaryAction = AppConfig.GetString(name + "_secondary");
        ...
        private static void HandleRogClick()
        {
            ...
            lock (rogClickLock)
            {
                ...
                if (rogClickCount == 1)
                {
                    rogClickTimer?.Dispose();
                    rogClickTimer = new System.Threading.Timer(_ => CompleteRogSingleClick(), null,
                        RogDoubleClickWindowMs, Timeout.Infinite);
                    return;
                }
            ...
            KeyProcess("m4_double");
        }

        private static void CompleteRogSingleClick()
        {
            lock (rogClickLock)
            {
                if (rogClickCount != 1) return;
                rogClickCount = 0;
                rogClickTimer?.Dispose();
                rogClickTimer = null;
            }
        ...
        private static void CancelPendingRogClick()
        {
            lock (rogClickLock)
            {
                rogClickCount = 0;
        ...
        public static void RunCustomAction(string action)
        {
            if (string.IsNullOrWhiteSpace(action)) return;
            if (!ExecuteAction(action.Trim(), null)) RunKeyCommand(action.Trim());
        }

        private static bool ExecuteAction(string action, string? configKey)
        {
            switch (action)
            {
                case "mute":
                ...
                case "screen":
                    ...
                    return true;
                case "miniled":
                    if (ScreenCCD.GetHDRStatus()) return true;
                ...
                case "overlay":
                    ...
                    return true;
                case "rtss_overlay":
                    Program.settingsForm.BeginInvoke(() => Program.settingsForm.ToggleRtssOverlay(true));
                    return true;
                case "micmute":
                    ToggleMic();
                    return true;
                ...
                case "brightness_down":
                    SetBrightness(false);
                    return true;
                case "fan_zero_toggle":
                    modeControl.ToggleFanStop80();
                    return true;
                case "fan_full_toggle":
                    modeControl.ToggleFanFullPower();
                    return true;
