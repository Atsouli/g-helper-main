...
namespace GHelper.UI
{
    public class RButton : Button
    {
        ...
        protected override void OnPaint(PaintEventArgs pevent)
        {
            ...
            if (Focused && Enabled)
            {
                int inset = Math.Max(3, border + 1);
                Rectangle focusRect = Rectangle.Inflate(ClientRectangle, -inset, -inset);
                using Pen focusPen = new Pen(Color.FromArgb(230, RForm.colorStandard), Math.Max(1.5f, ratio * 2f))
                {
                    DashStyle = DashStyle.Dot,
                    Alignment = PenAlignment.Inset
                };
                using GraphicsPath focusPath = GetFigurePath(focusRect, Math.Max(2, radius));
