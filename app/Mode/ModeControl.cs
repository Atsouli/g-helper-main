...
namespace GHelper.Mode
{
    public class ModeControl
    {
        ...
        public void AutoFans(bool force = false)
        {
            ...
            if (AppConfig.IsApplyFans() || AppConfig.IsMode("fan_stop_80") ||
                AppConfig.IsMode("fan_full_power") || force)
            {
                ...
                byte[] cpuCurve = ApplyFanStopMode(AppConfig.GetFanConfig(AsusFan.CPU));
                byte[] gpuCurve = ApplyFanStopMode(AppConfig.GetFanConfig(AsusFan.GPU));
                int cpuResult = Program.acpi.SetFanCurve(AsusFan.CPU, cpuCurve);
                int gpuResult = Program.acpi.SetFanCurve(AsusFan.GPU, gpuCurve);
                ...
                {
                    cpuResult = Program.acpi.SetFanRange(AsusFan.CPU, cpuCurve);
                    gpuResult = Program.acpi.SetFanRange(AsusFan.GPU, gpuCurve);
        ...
        public void ToggleFanStop80()
        {
            bool enabled = !AppConfig.IsMode("fan_stop_80");
            AppConfig.SetMode("fan_stop_80", enabled ? 1 : 0);
            if (enabled)
            {
                AppConfig.SetMode("fan_full_power", 0);
                AppConfig.SetMode("auto_apply", 1);
            }

            AutoFans(true);
            Program.toast.RunToast(enabled ? "0 RPM below 80°C: ON" : "0 RPM below 80°C: OFF",
                ToastIcon.Controller);
            Program.settingsForm.fansForm?.RefreshFanOverrideState();
        }

        public void ToggleFanFullPower()
        {
            bool enabled = !AppConfig.IsMode("fan_full_power");
            AppConfig.SetMode("fan_full_power", enabled ? 1 : 0);
            if (enabled)
            {
                AppConfig.SetMode("fan_stop_80", 0);
                AppConfig.SetMode("auto_apply", 1);
            }

            AutoFans(true);
            Program.toast.RunToast(enabled ? "Fan power 100%: ON" : "Fan power 100%: OFF",
                ToastIcon.Controller);
            Program.settingsForm.fansForm?.RefreshFanOverrideState();
        }

        public void SwitchFanExtremeMode()
        {
            if (AppConfig.IsMode("fan_stop_80"))
            {
                AppConfig.SetMode("fan_stop_80", 0);
                AppConfig.SetMode("fan_full_power", 0);
                ToggleFanFullPower();
        ...
        private static byte[] ApplyFanStopMode(byte[] source)
        {
            if (AsusACPI.IsInvalidCurve(source))
                return source;

            if (AppConfig.IsMode("fan_full_power"))
            {
                byte[] fullPower = (byte[])source.Clone();
                for (int index = 8; index < 16; index++)
                    fullPower[index] = 100;
                return fullPower;
            }

            if (!AppConfig.IsMode("fan_stop_80")) return source;

            byte[] curve = new byte[16];
            byte[] temperatures = { 30, 50, 65, 75, 79, 80, 90, 100 };
            Array.Copy(temperatures, curve, temperatures.Length);

            // Every firmware point below 80°C is explicitly zero. At the cutoff,
            // recover immediately and continue with an aggressive safety ramp.
            curve[13] = (byte)Math.Max(45, CurveSpeedAt(source, 80));
            curve[14] = (byte)Math.Max(75, CurveSpeedAt(source, 90));
            curve[15] = 100;
            return curve;
        }

        private static int CurveSpeedAt(byte[] curve, int temperature)
        {
            if (temperature <= curve[0]) return curve[8];

            for (int index = 1; index < 8; index++)
            {
                if (temperature > curve[index]) continue;
                int span = Math.Max(1, curve[index] - curve[index - 1]);
                int rise = curve[index + 8] - curve[index + 7];
                return curve[index + 7] + rise * (temperature - curve[index - 1]) / span;
            }
        ...
        public int AdjustTdpLimit(int delta)
        {
            int limitTotal = AppConfig.GetMode("limit_total", AsusACPI.DefaultTotal);
            int newLimit = Math.Clamp(limitTotal + delta, AsusACPI.MinTotal, AsusACPI.MaxTotal);

            if (newLimit == limitTotal) return newLimit;

            int limitSlow = AppConfig.GetMode("limit_slow", limitTotal);
            int limitFast = AppConfig.GetMode("limit_fast", limitTotal);
            int limitCpu = AppConfig.GetMode("limit_cpu", AsusACPI.DefaultCPU);

            AppConfig.SetMode("limit_total", newLimit);
            if (newLimit > limitSlow) AppConfig.SetMode("limit_slow", newLimit);
            if (newLimit > limitFast) AppConfig.SetMode("limit_fast", newLimit);
            if (newLimit < limitCpu) AppConfig.SetMode("limit_cpu", newLimit);

            Logger.WriteLine($"Gamepad TDP: {limitTotal}W -> {newLimit}W");
            return newLimit;
        }
