...
namespace GHelper
{
    public partial class Fans : RForm
    {
        ...
        public Fans()
        {
            ...
            panelApplyFans.Height += 96;
            checkFanStop80.AutoSize = false;
            checkFanStop80.Location = new Point(15, 34);
            checkFanStop80.Size = new Size(430, 42);
            checkFanStop80.Padding = new Padding(12, 4, 12, 4);
            checkFanStop80.Text = "0 RPM below 80°C (CPU + GPU)";
            checkFanStop80.AccessibleName = "Zero RPM fan mode below 80 degrees";
            checkFanStop80.TabIndex = 1;
            panelApplyFans.Controls.Add(checkFanStop80);

            checkFanFullPower.AutoSize = false;
            checkFanFullPower.Location = new Point(15, 78);
            checkFanFullPower.Size = new Size(430, 42);
            checkFanFullPower.Padding = new Padding(12, 4, 12, 4);
            checkFanFullPower.Text = "Full fan power 100% (CPU + GPU)";
            checkFanFullPower.AccessibleName = "Force CPU and GPU fans to full power";
            checkFanFullPower.TabIndex = 2;
            panelApplyFans.Controls.Add(checkFanFullPower);
            ...
            checkFanStop80.Click += CheckFanStop80_Click;
            checkFanFullPower.Click += CheckFanFullPower_Click;
        ...
        private void CheckFanStop80_Click(object? sender, EventArgs e)
        {
            AppConfig.SetMode("fan_stop_80", checkFanStop80.Checked ? 1 : 0);

            // This mode requires firmware fan-curve control. Enabling it also
            // enables custom fan application for the active performance profile.
            if (checkFanStop80.Checked)
            {
                checkFanFullPower.Checked = false;
                AppConfig.SetMode("fan_full_power", 0);
                checkApplyFans.Checked = true;
        ...
        private void CheckFanFullPower_Click(object? sender, EventArgs e)
        {
            AppConfig.SetMode("fan_full_power", checkFanFullPower.Checked ? 1 : 0);

            if (checkFanFullPower.Checked)
            {
                checkFanStop80.Checked = false;
        ...
        public void RefreshFanOverrideState()
        {
            if (IsDisposed || !IsHandleCreated) return;
            if (InvokeRequired)
            {
                BeginInvoke(RefreshFanOverrideState);
                return;
            }

            checkFanStop80.Checked = AppConfig.IsMode("fan_stop_80");
            checkFanFullPower.Checked = AppConfig.IsMode("fan_full_power");
            checkApplyFans.Checked = AppConfig.IsApplyFans();
        }
