...
public static class HardwareControl
{
    ...
    public static int? cpuClockMHz;
    public static int? gpuClockMHz;
    ...
    private struct SYSTEM_PROCESSOR_POWER_INFORMATION
    {
        public uint Number;
        public uint MaxMhz;
    ...
    private static int? GetCpuClockMHz()
    {
        int processorCount = Environment.ProcessorCount;
        int itemSize = Marshal.SizeOf<SYSTEM_PROCESSOR_POWER_INFORMATION>();
        IntPtr buffer = Marshal.AllocHGlobal(itemSize * processorCount);
        try
        {
            uint status = CallNtPowerInformation(ProcessorInformation, IntPtr.Zero, 0,
                buffer, (uint)(itemSize * processorCount));
            ...
            {
                var info = Marshal.PtrToStructure<SYSTEM_PROCESSOR_POWER_INFORMATION>(buffer + index * itemSize);
                if (info.CurrentMhz == 0) continue;
                total += info.CurrentMhz;
                valid++;
            }
            return valid > 0 ? (int)(total / valid) : null;
        }
        catch { return null; }
    ...
    public static void ReadSensorsOverlay()
    {
        ...
        if (readMemory)
        {
            ...
            try
            {
                ...
                if (vram is { } v && v.totalMb > 0)
                {
                    ...
                    vramTotalMb = (int)v.totalMb;
                    vramUsage = (int)Math.Clamp(v.usedMb * 100 / v.totalMb, 0, 100);
                }
                else { vramUsedMb = null; vramTotalMb = null; vramUsage = null; }
            }
            catch { vramUsedMb = null; vramTotalMb = null; vramUsage = null; }
        }
        else
        {
            ramUsage = null;
        ...
        if (readClocks)
        {
            cpuClockMHz = GetCpuClockMHz();
            try
            {
                gpuClockMHz = GpuControl?.GetGpuClockMHz();
                if (gpuClockMHz is null && isAMDiGPU)
                    gpuClockMHz = AmdApu().GetGpuClockMHz();
            }
            catch { gpuClockMHz = null; }
        }
        else
        {
            cpuClockMHz = null;
    ...
    public static int GetBatteryRemainingTimeSeconds()
    {
        try
        {
            SYSTEM_POWER_STATUS status = default;
            if (GetSystemPowerStatus(ref status) && status.ACLineStatus == 0 && status.BatteryLifeTime > 0)
                return status.BatteryLifeTime;
        }
        catch (Exception ex)
        {
            Debug.WriteLine("Battery Remaining Time Reading: " + ex.Message);
        }

        // Windows does not always provide BatteryLifeTime. Estimate it from the
        // current remaining capacity and discharge rate when both are available.
        if (chargeCapacity > 0 && batteryRate < -0.5m)
        {
            decimal hours = chargeCapacity.Value / 1000m / Math.Abs(batteryRate.Value);
            return (int)Math.Round(hours * 3600m);
        }
