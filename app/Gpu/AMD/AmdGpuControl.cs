...
public class AmdGpuControl : IGpuControl
{
    ...
    public int? GetGpuClockMHz()
    {
        if (IsValid && GetPMLog(out ADLPMLogDataOutput discreteLog))
            return Sensor(discreteLog, ADLSensorType.PMLOG_CLK_GFXCLK);
        if (GetPMLogiGpu(out ADLPMLogDataOutput integratedLog))
            return Sensor(integratedLog, ADLSensorType.PMLOG_CLK_GFXCLK);
        return null;
    }
