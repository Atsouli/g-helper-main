...
public static class NvmlHelper
{
    ...
    [DllImport(NvmlDll)] static extern int nvmlDeviceGetClockInfo(IntPtr device, uint clockType, out uint clockMHz);

    [StructLayout(LayoutKind.Sequential)]
    struct nvmlMemory_t { public ulong total; public ulong free; public ulong used; }

    const uint NVML_TEMPERATURE_GPU = 0;
    const uint NVML_CLOCK_GRAPHICS = 0;
    ...
    public static int? GetGraphicsClock(uint gpuIndex = 0)
    {
        lock (_lock)
        {
            Init();
            ...
            {
                if (nvmlDeviceGetHandleByIndex_v2(gpuIndex, out IntPtr device) != NVML_SUCCESS) return null;
                if (nvmlDeviceGetClockInfo(device, NVML_CLOCK_GRAPHICS, out uint clock) != NVML_SUCCESS) return null;
                return clock > 0 ? (int)clock : null;
            }
            catch { return null; }
        }
    }
