    ...
    public class HardwareOverlay : OSDNativeForm
    {
        ...
        private void Tick()
        {
            ...
            if (_showBattery)
            {
                ...
                if (_onBattery)
                {
                    double level = HardwareControl.batteryCapacity > 0
                        ? (double)HardwareControl.batteryCapacity
                        : HardwareControl.GetBatteryChargePercentage();
                    _batPercent = (int)Math.Round(level);
                    _batLevel = level > 0 ? level.ToString("F1", CultureInfo.InvariantCulture) + "%" : "--";
